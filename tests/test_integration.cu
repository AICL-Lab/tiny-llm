...
class IntegrationTest : public ::testing::Test {
  protected:
    ...
    half *randomDeviceFP16(int size, float scale = 1.0f, unsigned seed = 42) {
        ...
        cudaMemcpy(d_data, h_data.data(), size * sizeof(half), cudaMemcpyHostToDevice);
        return d_data;
    }
    ...
    int8_t *randomDeviceINT8(int size, unsigned seed = 123) {
        ...
        cudaMemcpy(d_data, h_data.data(), size * sizeof(int8_t), cudaMemcpyHostToDevice);
        return d_data;
    }

    // Create random quantized weight
    QuantizedWeight createRandomWeight(int rows, int cols, int group_size, unsigned seed) {
        QuantizedWeight w;
        w.rows = rows;
        w.cols = cols;
...
TEST_F(IntegrationTest, TemperatureAffectsSampling) {
    ...
    for (int i = 0; i < 100; ++i) {
        int result = InferenceEngine::sampleTemperature(logits.data(), vocab_size, 0.1f, i);
    ...
    for (int i = 0; i < 100; ++i) {
        int result = InferenceEngine::sampleTemperature(logits.data(), vocab_size, 2.0f, i + 1000);
