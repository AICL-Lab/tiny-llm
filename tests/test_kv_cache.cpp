...
class KVCacheTest : public ::testing::Test {
  protected:
    ...
    // Helper to create KVCacheManager using factory method
    std::unique_ptr<KVCacheManager> createCache(const KVCacheConfig &config) {
        auto result = KVCacheManager::create(config);
        EXPECT_TRUE(result.isOk()) << "Failed to create KVCacheManager: " << result.error();
