...
class Validator {
  public:
    ...
    static Result<void> validateTokenId(int token_id, int vocab_size,
        ...
        if (vocab_size <= 0) {
            return Result<void>::err(
                buildContext(context, "Invalid vocab_size: " + std::to_string(vocab_size) +
                                          " (must be positive)"));
        }
        if (token_id < 0 || token_id >= vocab_size) {
            return Result<void>::err(buildContext(
                context, "Invalid token ID: " + std::to_string(token_id) + " (valid range: 0-" +
     ...
     * @return Result<void> indicating success or error with message
     */
    static Result<void> validateTokenSequence(const std::vector<int> &tokens, int vocab_size,
                                              const std::string &context = "") {
        if (tokens.empty()) {
            return Result<void>::err(buildContext(context, "Token sequence is empty"));
        }
        for (size_t i = 0; i < tokens.size(); ++i) {
            auto result = validateTokenId(tokens[i], vocab_size,
     ...
     * @return Result<void> indicating success or error with message
     */
    static Result<void> validateGenerationConfig(const GenerationConfig &config) {
        ...
        if (config.top_k < 0) {
            return Result<void>::err("top_k must be non-negative: " + std::to_string(config.top_k));
        }
        if (config.top_p <= 0.0f || config.top_p > 1.0f) {
            return Result<void>::err("top_p must be in (0, 1]: " + std::to_string(config.top_p));
        }
    ...
    static Result<void> validateModelConfig(const ModelConfig &config) {
        ...
        if (config.num_heads % config.num_kv_heads != 0) {
            return Result<void>::err(
                "num_heads must be divisible by num_kv_heads: " + std::to_string(config.num_heads) +
                " % " + std::to_string(config.num_kv_heads) + " != 0");
        }
        if (config.hidden_dim % config.num_heads != 0) {
            return Result<void>::err(
                "hidden_dim must be divisible by num_heads: " + std::to_string(config.hidden_dim) +
     ...
     * @return Result<void> indicating success or error with message
     */
    static Result<void> validatePromptLength(int prompt_len, int max_new_tokens, int max_seq_len) {
        if (prompt_len <= 0) {
            return Result<void>::err("Prompt length must be positive: " +
                                     std::to_string(prompt_len));
        }
        if (prompt_len >= max_seq_len) {
            return Result<void>::err("Prompt length " + std::to_string(prompt_len) +
                                     " exceeds max_seq_len " + std::to_string(max_seq_len));
        }
        int total_len = prompt_len + max_new_tokens;
        if (total_len > max_seq_len) {
            return Result<void>::err(
                "Total length (prompt + max_new_tokens = " + std::to_string(total_len) +
                ") exceeds max_seq_len " + std::to_string(max_seq_len));
        }
        return Result<void>::ok();
    }
     ...
     */
    template <typename T>
    static Result<void> validateNotNull(const T *ptr, const std::string &name) {
        if (ptr == nullptr) {
    ...
    static Result<void> validateLayerIndex(int layer_idx, int num_layers,
        ...
        if (layer_idx < 0 || layer_idx >= num_layers) {
            return Result<void>::err(buildContext(
                context, "Invalid layer_idx: " + std::to_string(layer_idx) + " (valid range: 0-" +
  ...
  private:
     ...
     */
    static std::string buildContext(const std::string &context, const std::string &message) {
        if (context.empty()) {
            return message;
        }
