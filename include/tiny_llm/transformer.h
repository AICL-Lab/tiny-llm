...
class TransformerLayer {
  public:
    ...
    void forward(half *hidden_states, KVCacheManager &kv_cache, int seq_id, int position,
                 cudaStream_t stream = 0);

    // Forward pass for multiple tokens (prefill phase)
    // hidden_states: [batch_size, seq_len, hidden_dim] - input and output
    void forwardPrefill(half *hidden_states, KVCacheManager &kv_cache, int seq_id, int seq_len,
                        cudaStream_t stream = 0);
  ...
  private:
    // Attention sublayer
    void attention(const half *x, half *output, KVCacheManager &kv_cache, int seq_id, int position,
