...
__global__ void rmsnorm_kernel(const half *__restrict__ input, const half *__restrict__ weight,
                               half *__restrict__ output, int hidden_dim, float eps) {
    int row = blockIdx.x;
    int tid = threadIdx.x;
...
void rmsnorm(const half *input, const half *weight, half *output, int batch_size, int hidden_dim,
    ...
    rmsnorm_kernel<<<batch_size, block_size, 0, stream>>>(input, weight, output, hidden_dim, eps);
}

// In-place version
__global__ void rmsnorm_inplace_kernel(half *__restrict__ x, const half *__restrict__ weight,
...
void rmsnorm_inplace(half *x, const half *weight, int batch_size, int hidden_dim, float eps,
                     cudaStream_t stream) {
    if (batch_size <= 0 || hidden_dim <= 0) {
        return;
    }
    ...
    rmsnorm_inplace_kernel<<<batch_size, block_size, 0, stream>>>(x, weight, hidden_dim, eps);
}
